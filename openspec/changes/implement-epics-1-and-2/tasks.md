...
Sections 5-8 form one releasable HU-04/HU-05 slice. Section 8.7's stricter eligibility must ship with completion/discovery, not alone. Section 9 covers the care relationship portion of HU-06. Former sections 10-12 (intervention/audit storage, the audited clinical interface, and epic completion/evidence) are removed from this queue and deferred until later. Their specification and design contracts remain future requirements; do not implement them or claim full epic completion from this queue. Required verification for each retained task and repository validation rules still apply. Later sections depend on earlier sections unless a task states a narrower prerequisite. Commands use PostgreSQL; never substitute SQLite.
...
- [x] 5.1 Implement D3 professional input validation in professionals/validators.py, reusing existing DNI/date validators. Verify professionals/test_validation.py covers surrounding DNI whitespace, preserved leading zeroes, rejected punctuation/Unicode/invalid lengths, trimmed nonblank names up to 150 characters, future dates, and active reference inputs with `uv run pytest professionals/test_validation.py -q`.
- [x] 5.2 Extend the existing Professional in professionals/models.py with D2 nullable fields and named PostgreSQL check/unique constraints; add a migration after current migrations. Verify professionals/test_professional_model.py covers incomplete rows, required completed fields, active-only DNI uniqueness and protected user/reference links; do not recreate IDs or edit old migrations.
- [x] 5.3 Add sequence migration and professionals/identifiers.py generate_registration_number using the existing patients/identifiers.py pattern; allocate only at successful creation/completion. Verify professionals/test_identifiers.py tests format, repeated and dedicated-connection concurrent uniqueness, no number on invalid input, and preservation of an assigned number.
- [x] 5.4 Add professionals/test_migrations.py with an old-schema active profile, inactive profile, User links, and a linked Admission. Apply new migrations and verify IDs, flags, FKs, row counts are unchanged and new personal fields are NULL; test empty-database reverse/forward without deleting historical data.
- [x] 5.5 Add Professional completeness/display helpers needed by forms and lists without enabling stricter clinical access yet. Verify model tests distinguish incomplete active, incomplete inactive, complete active, and complete inactive profiles without inferring completeness from is_active alone.
...
- [x] 7.1 Implement D4 account resolution and shared administrative authorization in professionals/services.py; require a known active username without requiring an existing medical group. Verify professionals/test_services.py denies missing/wrong/inactive actors, rejects unknown/disabled subject accounts, and never creates passwords/accounts/staff privileges.
- [x] 7.2 Implement register_professional for a new account using D2-D4, atomic profile+medical-group writes, server-owned number/completion timestamp, and database conflict handling. Verify service tests cover success, invalid fields, inactive references, account/DNI conflict, and no partial group/profile writes.
- [x] 7.3 Extend register_professional to complete an existing incomplete profile in place while preserving its active flag and admission links. Verify active/inactive legacy completion, missing fields, already-completed conflict, stable registration number, and rollback of failed completion in professionals/test_services.py.
- [x] 7.4 Add a real PostgreSQL interleaving regression in professionals/test_concurrency.py for competing registrations/completions and DNI collisions after initial checks. Verify one identity/group result, field-specific conflict, no 500, and safe reference-state recheck; follow the dedicated-connection patient tests, not sleep-only assertions.
- [x] 7.5 Implement update_professional with the D4 allowlist, current-state reload/lock, active-new-reference validation, and immutable identity rejection. Verify professionals/test_services.py covers each allowed field, identity/status injection, rejected incomplete profiles, inactive completed edits that preserve inactivity, replacement of retired references, retaining an unchanged retired reference during unrelated edits, and a stale active object after deactivation that cannot reactivate the current row.
- [x] 7.6 Implement confirmed idempotent deactivate_professional and confirmed reactivate_professional with current account/reference/DNI checks. Verify unchanged IDs, rejected unconfirmed/disabled-account/inactive-reference/DNI-conflicting reactivation, preserved history, and refreshed return state; relationship cleanup is added in 9.5.
- [x] 7.7 Implement the shared D4 eligibility predicate without wiring all entry points yet. Verify access_control/test_professional_eligibility.py covers active User/group/profile/completeness independently, dual roles, and superuser-without-medical-role; no session-cached eligibility.

## 8. HU-04/HU-05 - API, discovery, and compatibility rollout

- [x] 8.1 Add professional registration/detail/search serializers and API routes from D5, including inclusion in helixhealth/urls.py. Require `license_number` for registration/completion, return it as text or null in detail/search, and retain every existing response field including the generated `registration_number`. Verify professionals/test_api.py asserts exact fields/statuses, read-only/unknown-key rejection, canonical DNI collisions, missing/invalid license rejection without partial writes, empty/found search envelopes with both `registration_number` and `license_number`, authorization before lookup, and no raw exception leakage.
- [x] 8.2 Add professional PATCH, deactivate, and reactivate API views using D4 services, session authentication, and CSRF. PATCH accepts `license_number` as mutable, preserves it when omitted, and rejects explicit null/blank values; it never changes the generated `registration_number`. Verify API tests cover license correction and legacy-null output, inactive/incomplete state, explicit confirmation, missing target, forbidden roles, conflicts, and successful persisted responses; PUT/DELETE stay 405.
- [x] 8.3 Implement exact-DNI lookup, index queries, and 20-row active/inactive/incomplete index contexts in professionals/services.py and views.py. Include the entered License number independently from the generated Registration number in list/search output and show Not recorded for a legacy null license. Verify professionals/test_search.py checks surrounding search whitespace, punctuation rejection, inactive exclusion, tab completeness rules, stable ordering, page boundaries, index-backed lookup, and both number displays.
- [x] 8.4 Add Professionals navigation, index/search/result/detail templates, and D5 routes. Verify professionals/test_views.py for least-privileged navigation, one-action detail access, prefilled unmatched registration, inactive/incomplete discovery, and correct actions for each state.
- [x] 8.5 Add new/completion registration forms and templates with fixed username on completion and bound errors. Verify field/conflict feedback, HTMX 422 swapping, ordinary POST redirect, and persisted IDs in professionals/test_views.py.
- [x] 8.6 Add professional edit, deactivate, and reactivate forms/templates from D5, including Edit on inactive completed details to repair references or correct License number before reactivation. A completed legacy null license displays Not recorded; an untouched blank Edit control preserves it, while an explicit attempt to clear a known license is rejected. Verify confirmation errors, preserved identifiers/generated Registration number/history/activation state, rejected inactive-reference/DNI-conflicting reactivation, and visible active/inactive/incomplete states.
...
- [x] 8.8 Add B1/B2/B3 isolated browser journeys to tests/test_browser_workflows.py: non-staff admin starts at /, registers an account without profile/medical role, completes an old identity, and sees invalid-registration feedback; a fresh medical login proves eligibility. Verify persisted identity/group/admission links and that inactive completion stays inactive; do not seed the state being established.
- [x] 8.9 Add B4/B8 professional maintenance/search/pagination browser journeys, including separate License number/Registration number display, license correction, legacy Not recorded/Edit preservation, invalid edits, confirmation, denied inactive-reference/DNI-conflicting reactivation, 21-record pages, and return to saved detail. Verify `uv run pytest -m browser -q` executes these with no skipped cases.
- [x] 8.10 Extend seed_acceptance, verify_acceptance, and run_compose_acceptance.py for B1-B4/B8 using supported registration with explicit synthetic licenses for unrelated background professionals. Replace the old missing-profile auto-activation expectation with denied-then-administrator-completed behavior. Exercise legacy license addition through visible controls rather than prepopulating it, and verify the disposable live validator preserves generated Registration numbers, clinical eligibility, old admissions, every existing patient field, and patient journeys.
- [x] 8.11 Document HU-04 upgrade/registration/reactivation operator steps in docs/acceptance/sprint-3-hu-04-hu-05.md, including required new License number input, its distinction from the generated Registration number, legacy Not recorded/Edit behavior, and existing inactive and no-role account cases. Verify every documented action corresponds to a visible control exercised by 8.8-8.10; never claim incomplete legacy data or licenses were backfilled automatically.
