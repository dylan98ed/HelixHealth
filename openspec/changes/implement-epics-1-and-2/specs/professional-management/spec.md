...
- **WHEN** an administrator supplies complete valid data for an existing account with no profile
...
- **WHEN** a Django Admin operator inspects professionals
- **THEN** professional add/change/delete actions are unavailable in Django Admin and professional lifecycle changes use the product workflow

### Requirement: HU-04 - Complete legacy identities without losing history
The system SHALL retain existing professional IDs, account links, active flags, and admissions when registration fields are added. It SHALL NOT fabricate missing personal data. Administrators SHALL discover incomplete profiles through an Incomplete list and complete them in place. Missing/incomplete profiles SHALL NOT gain clinical access merely through medical role membership after HU-04 rollout.

#### Scenario: Complete an active legacy profile
- **WHEN** an administrator completes an existing active incomplete profile linked to an admission
...
- **THEN** the application explains that registration/reactivation is required and neither auto-completes the profile nor grants clinical navigation

### Requirement: HU-04 - Enforce canonical unique identity
The system SHALL permit at most one active professional per canonical DNI. Completed identity values (account, internal ID, DNI, registration number) SHALL be immutable.

#### Scenario: Invalid DNI spelling
- **WHEN** a DNI contains internal whitespace, punctuation, non-ASCII digits, or is not 7 or 8 digits long after trimming
- **THEN** validation rejects it without creating or changing a professional

#### Scenario: Concurrent identity collision
- **WHEN** two requests compete for the same account or active DNI
...
- **WHEN** an inactive professional's DNI is used for a new account
...
- **THEN** the saved detail shows the changes while account/DNI/internal ID/registration number stay unchanged

#### Scenario: Attempt identity or status injection
- **WHEN** a client submits an immutable identifier, activation/completion field, or unknown writable field to the update API
...
- **WHEN** an administrator confirms reactivation and all current account/reference/identity checks succeed
...
- **WHEN** confirmation is absent or account/reference/DNI checks fail
...
- **THEN** the references are saved while the profile remains inactive, and a separate confirmed reactivation can proceed through current account and DNI checks
...
- **THEN** the result shows full name and generated registration number with a one-action detail link
...
- **THEN** no professional data or mutation is exposed; anonymous HTML reads go through the application login
...
A professional SHALL be permitted intervention access only while the account is active, medical role is present, registration is complete, profile is active, patient is active, and an active care relationship exists. My patients SHALL show only currently accessible patients and provide visible View interventions links. Original authorship or staff/superuser flags alone SHALL NOT bypass these conditions.
...
- **THEN** the event stores available authenticated identity and requested references, leaves unknown identities null, and discloses no clinical data to the requester
